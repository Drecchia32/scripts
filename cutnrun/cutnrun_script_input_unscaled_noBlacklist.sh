...
bedtools intersect -v -abam $PROJECTPATH/mapped/${base%R1_001.fastq.gz}bt2.mm10.mapped.bam -b /hpc/uu_epigenetics/davide/annotations/blacklists/mm10_mitochondrial_blacklist/empty_blacklist.bed > $PROJECTPATH/mapped/${base%R1_001.fastq.gz}bt2.mm10.mapped.blacklistFiltered.bam
...
bedtools genomecov -bg -scale $ScalingFactor -i $PROJECTPATH/mapped/bed/${base%R1_001.fastq.gz}bt2.mm10.fragments.bed -g /hpc/uu_epigenetics/davide/annotations/mm10.chrom.sizes > $PROJECTPATH/bedgraph/${base%R1_001.fastq.gz}bt2.mm10.fragments.noBlacklist.manualCPM.bedgraph

#convert from bedgraaph to bigWig
#bigwig is normalized, becaause the bedgraph file was normalized in the last step
bedGraphToBigWig $PROJECTPATH/bedgraph/${base%R1_001.fastq.gz}bt2.mm10.fragments.manualCPM.bedgraph /hpc/uu_epigenetics/davide/annotations/mm10.chrom.sizes $PROJECTPATH/bigwig/${base%R1_001.fastq.gz}bt2.mm10.fragments.noBlacklist.manualCPM.bigwig
