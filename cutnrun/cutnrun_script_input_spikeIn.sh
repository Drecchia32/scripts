...
#removing trimming has temporarily been moved to later i nthe script to accomodate for testin the psikin normalizaiotn
...
echo "spikeIn scaling"
####Spike-in Scaling
spikeInRef="/hpc/uu_epigenetics/davide/annotations/ecoli/ecoli_K12-MG1655" #add reff
chromSize="" #download and add ecoli chrom size

echo "aligning to spikeIn genome"
bowtie2 --end-to-end --very-sensitive --no-mixed --no-discordant --phred33 -I 10 -X 700 -p $((CORES)) -x ${spikeInRef} -1 $PROJECTPATH/trimmed/${base%R1_001.fastq.gz}R1_001_val_1.fq.gz -2 $PROJECTPATH/trimmed/${base%R1_001.fastq.gz}R2_001_val_2.fq.gz -S $PROJECTPATH/mapped/${base%R1_001.fastq.gz}bt2.spikeIn.sam &> $PROJECTPATH/mapped/${base%R1_001.fastq.gz}.bt2.spikeIn.report.txt

echo "finding seqDepthDouble"
#why double? is it because we're not looking at fragments here, but both mates of a pair?
#excludes unmapped reads (-F 0x04). this doesn't specify anything about mate-pairs, which is why this is "doubled"
seqDepthDouble=`samtools view -F 0x04 $PROJECTPATH/mapped/${base%R1_001.fastq.gz}bt2.spikeIn.sam | wc -l`

#account for paired-end reads
seqDepth=$((seqDepthDouble/2))

#save sequencing depth for spikeIn normalization to txt
echo $seqDepth >$PROJECTPATH/bedgraph/${base%R1_001.fastq.gz}seqDepth_spikeIn.txt

ScalingFactor_SpikeIn=$(bc <<< "scale=6;10000/$seqDepth")

#save spikein scaling factor
echo $ScalingFactor_SpikeIn >$PROJECTPATH/bedgraph/${base%R1_001.fastq.gz}ScalingFactor_spikeIn.txt
#do something about the rm of trimmed files
...
bedtools genomecov -bg -scale $ScalingFactor_SpikeIn -i $PROJECTPATH/mapped/bed/${base%R1_001.fastq.gz}bt2.mm10.fragments.bed -g /hpc/uu_epigenetics/davide/annotations/mm10.chrom.sizes > $PROJECTPATH/bedgraph/${base%R1_001.fastq.gz}bt2.mm10.fragments.spikeIn.bedgraph

#convert from bedgraaph to bigWig
#bigwig is normalized, becaause the bedgraph file was normalized in the last step
bedGraphToBigWig $PROJECTPATH/bedgraph/${base%R1_001.fastq.gz}bt2.mm10.fragments.spikeIn.bedgraph /hpc/uu_epigenetics/davide/annotations/mm10.chrom.sizes $PROJECTPATH/bigwig/${base%R1_001.fastq.gz}bt2.mm10.fragments.spikeIn.bigwig
